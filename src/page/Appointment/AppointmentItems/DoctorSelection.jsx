...
function DoctorSelection({ formData, errors, onChange }) {
  ...
  useEffect(() => {
    const fetchDoctors = async () => {
      try {
        ...
        // Có thể là response.data.listAllDoctors hoặc response.data.data.listAllDoctors
        const doctorsRaw =
          response.data.listAllDoctors ||
          response.data.data?.listAllDoctors ||
          [];
        ...
        const activeDoctors = doctorsRaw.filter((item) => item.status === 1);

        const transformedDoctors = activeDoctors.map((doctor) => {
          const education =
            doctor.certificates?.map((cert) => cert.certificate) || [];

          // Ensure consultationTypes is always an array
          let consultationTypes = [];
          if (doctor.certificates && doctor.certificates.length > 0) {
            consultationTypes = doctor.certificates.map(
              (cert) => cert.specialization
            );
          }
      ...
      const filtered = allDoctors.filter(doctor => {
        ...
        return consultationTypes.some(
          (type) =>
            ...
            (type
              .toLowerCase()
              .includes(formData.consultationType.toLowerCase()) ||
  ...
  const fetchDoctorTimeSlots = async (doctorId) => {
    try {
      console.log(
        `🕒 Fetching available time slots for doctor ID: ${doctorId}`
      );
      ...
      if (cycleCount >= maxCycles) {
        ...
        setTimeout(async () => {
          const finalRandomIndex = Math.floor(
            Math.random() * filteredDoctors.length
          );
          const finalRandomDoctor = filteredDoctors[finalRandomIndex];

          onChange({
            target: { name: "doctor_id", value: finalRandomDoctor.id },
          });
          onChange({
            target: { name: "doctorName", value: finalRandomDoctor.name },
          });
          onChange({ target: { name: "preferredTime", value: "" } });
          ...
          console.log(
            `🎲 Đã chọn ngẫu nhiên bác sĩ: ${finalRandomDoctor.name}`
          );
        }, 300);
      }
    }, 120);
  ...
  return (
    <div className={cx("form-section", "doctor-selection")}>
      <div className={cx("section-header")}>
        <h3 className={cx("section-title")}>👨‍⚕️ Chọn bác sĩ tư vấn</h3>
      </div>
